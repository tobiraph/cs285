...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        discounts = np.array([np.power(self.gamma,i) for i in range(len(rewards))])
        list_of_discounted_returns = [np.sum(discounts*rewards)]*len(rewards)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for t, _ in enumerate(rewards):
            discounts = np.array([np.power(self.gamma,t_bar-t) for t_bar in range(t,len(rewards))])
            rs = np.array(rewards[t:])
            discounted_cumsum = np.sum(discounts*rs)
