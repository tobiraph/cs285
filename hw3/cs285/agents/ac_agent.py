...
class ACAgent(BaseAgent):
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # query the critic with ob_no, to get V(s)
        v_s = self.critic.forward_np(ob_no)
        # query the critic with next_ob_no, to get V(s')
        v_s_bar = self.critic.forward_np(next_ob_no)
        # cut off the V(s') term (ie set it to 0) at terminal states (ie terminal_n=1)
        v_s_bar[terminal_n==1]=0
        # estimate the Q value as Q(s, a) = r(s, a) + gamma*V(s')
        q_n = re_n + self.gamma * v_s_bar
        # calculate advantage (adv_n) as A(s, a) = Q(s, a) - V(s)
        adv_n = q_n - v_s
